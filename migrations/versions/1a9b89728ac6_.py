...
Revision ID: 1a9b89728ac6
Revises: 
Create Date: 2020-04-22 09:54:39.224625
...
def upgrade():
    ...
    sa.Column('step', sa.Enum('NONE', 'CLONE', 'BUILD', 'DEPLOY', 'CLEAN', name='buildstep'), nullable=False),
    sa.Column('roman_step', sa.String(), nullable=False),
    sa.Column('result', sa.Boolean(), nullable=True),
    sa.Column('start_time', sa.DateTime(), nullable=True),
    sa.Column('end_time', sa.DateTime(), nullable=True),
    sa.Column('log_text', sa.Text(), nullable=True),
    sa.PrimaryKeyConstraint('course_id', 'number', 'step', 'roman_step', name=op.f('pk_build_log'))
    )
    with op.batch_alter_table('build_log', schema=None) as batch_op:
        batch_op.create_index('build_id_number', ['course_id', 'number'], unique=False)

    op.create_table('git_repository',
    sa.Column('origin', sa.String(length=255), nullable=False),
    sa.Column('public_key', sa.Text(), nullable=True),
    sa.Column('last_validation', sa.DateTime(), nullable=True),
    sa.Column('state', sa.Enum('VALID', 'NO_MATCHING_PAIR', 'NO_ACCESS_TO_REMOTE', name='state'), nullable=True),
    ...
    sa.ForeignKeyConstraint(['origin'], ['git_repository.origin'], name=op.f('fk_course_instance_origin_git_repository')),
    ...
    sa.Column('start_time', sa.DateTime(), nullable=True),
    sa.Column('end_time', sa.DateTime(), nullable=True),
    sa.Column('state', sa.Enum('NONE', 'PUBLISH', 'RUNNING', 'SUCCESS', 'FAILED', 'CANCELED', name='buildstate'), nullable=True),
    sa.Column('step', sa.Enum('NONE', 'CLONE', 'BUILD', 'DEPLOY', 'CLEAN', name='buildstep'), nullable=True),
    sa.ForeignKeyConstraint(['course_id'], ['course_instance.id'], name=op.f('fk_build_course_id_course_instance')),
    sa.PrimaryKeyConstraint('course_id', 'number', name=op.f('pk_build'))
    )
...
def downgrade():
    ...
    with op.batch_alter_table('build_log', schema=None) as batch_op:
        batch_op.drop_index('build_id_number')
